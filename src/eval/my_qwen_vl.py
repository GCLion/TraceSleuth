...
class InconsistencyAttentionModule(nn.Module):
    def __init__(self, dim_vis, num_heads=4):
        super().__init__()
        self.att_linear = nn.Linear(dim_vis, dim_vis)
        self.num_heads = num_heads

    def forward(self, feat_vis):
        # feat_vis: [N, dim_vis]
        N, D = feat_vis.shape
        Q = self.att_linear(feat_vis)
        K = self.att_linear(feat_vis)
        att = torch.softmax(Q @ K.T / (D**0.5), dim=-1)  # [N, N]
        
        # Measure deviation from uniform / expected consistency
        inconsistency = torch.mean(torch.abs(att - att.mean(dim=-1, keepdim=True)), dim=-1)
        return inconsistency.unsqueeze(-1)  # [N, 1]
