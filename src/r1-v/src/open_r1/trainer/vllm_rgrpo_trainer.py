...
class AgentVLMVLLM:
    ...
    def process(self, image_path, question, max_iterations=1, sample_id=None):#max_iterations=8
        """
        ...
        while iterations < max_iterations:
            ...
            ##################
            # 把输入移动到模型所在设备
            # print("##DEBUG model.device:", next(self.model.parameters()).device)
            # print("##DEBUG self.device:", self.device)
            inputs = {k: v.to(self.device) for k, v in inputs.items()}

            # 使用 Transformers 的 generate
            gen_kwargs = {
                "max_new_tokens": self.max_tokens_once,
                "temperature": self.temperature,
                "do_sample": True,
            }
            ...
            # output_ids 形状通常是 [1, prompt_len + gen_len]
            # 取新生成的部分（去掉 prompt 部分）
            prompt_len = inputs["input_ids"].shape[1]
            generated_ids_trimmed = output_ids[0][prompt_len:]

            # 解码为文本
            generated_text = self.processor.tokenizer.decode(
                generated_ids_trimmed,
                skip_special_tokens=False,  # 保留 <answer> 等特殊标签，后面还要用
            )
...
class Qwen2VLRGRPOVLLMTrainer(Trainer):
    ...
    def _prepare_inputs(
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                res = []
                agent = AgentVLMVLLM(self.llm, self.processor, min_pixels=self.min_pixels, max_pixels=self.max_pixels)
            ...
            mutimodal_inputs = self.processor(
                text=[all_text],
                images=image_inputs,
                videos=video_inputs,
                return_tensors="pt",
