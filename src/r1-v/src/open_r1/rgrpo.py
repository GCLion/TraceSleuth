...
from trainer import Qwen2VLGRPOTrainer, Qwen2VLRGRPOVLLMTrainer
...
def main(script_args, training_args, model_args):
    ...
    print("Loaded samples from json:", len(all_data))
    print("First raw item:", all_data[0])

    dataset = Dataset.from_list(all_data)
    print("Dataset length after from_list:", len(dataset))

    dataset = dataset.map(make_conversation_from_jsonl, num_proc=1)
    print("Dataset length after map:", len(dataset))
    print("Example 0 after map:", dataset[0])
    ...
    ####
    if hasattr(trainer, "model") and hasattr(trainer.model, "destroy"):
